...
const CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRgCy7ocMnHhbaPRFQW7pfCLjbiKf6g3elNQBYVw5kSOZqqMx-cvBa8B3ILuZdMcx6Af0sMO6h--6lX/pub?output=csv';
...
// Função para garantir que a data seja lida no fuso local (evita o atraso de -1 dia)
...
export function useCampaignData() {
  ...
  // Estado inicial: Últimos 7 dias (D-7 até Ontem)
  const [dateRange, setDateRange] = useState<DateRange>(() => {
    const to = endOfDay(subDays(new Date(), 1)); // Ontem
    const from = startOfDay(subDays(to, 6));     // 7 dias contando com ontem
    return { from, to };
  });
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Normalização imediata para tratar o fuso horário
