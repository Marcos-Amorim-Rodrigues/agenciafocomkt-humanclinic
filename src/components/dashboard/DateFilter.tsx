...
export function DateFilter({ dateRange, onDateRangeChange, availableDateRange }: DateFilterProps) {
  ...
  const handlePreset = (days: number) => {
    // 1. Define ONTEM como a data final de referência (D-1)
    const ontem = subDays(new Date(), 1);
    const to = endOfDay(ontem);
    
    // 2. Define o início voltando a quantidade de dias (ex: 7 dias = ontem + 6 anteriores)
  ...
  const handleSelect = (range: DayPickerDateRange | undefined) => {
    ...
    if (range?.from && range?.to) {
      // Normaliza a seleção manual para cobrir o dia inteiro
      onDateRangeChange({ 
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        ...
        <PopoverContent className="w-auto p-0 bg-popover border-border" align="start">
          <Calendar
            ...
            numberOfMonths={2}
            disabled={(date) => {
              // Bloqueia seleção de hoje ou datas futuras
